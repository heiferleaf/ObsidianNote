...
本篇笔记的内容为：
1. 通过Kafka实现消息队列，从而实现业务逻辑的异步处理，起到代码解耦、提高响应时间，削峰填谷的作用，也带来了编程代码的复杂，调试困难的问题。此外，还需要额外注意，同一个用户发送的有逻辑顺序的消息，需要被存储在通过一个Topic的同一个分区中，发送消息时，设计确认机制，防止消息丢失，设计生产者和消费者的幂等性，和状态机。
